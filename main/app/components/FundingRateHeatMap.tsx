...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  return (
    ...
    >
      <div>
        <strong>Exchange:</strong> {content.exchange.slice(0, -5).toUpperCase()}
      </div>
      <div>
        <strong>Asset:</strong> {content.asset}
      </div>
      <div>
        <strong>Funding Rate:</strong> {formatFundingRate(content.value, isAPR)}
      </div>
      <div>
        <strong>Time:</strong> {content.timestamp.slice(0, -8)}
      </div>
    </div>
  );
};
...
const FundingRateHeatMap: React.FC<FundingRateHeatMapProps> = ({
  ...
  const { exchanges, assets, timestamps, fundingRates } = useMemo(() => {
    ...
    const timestamps = data
      .map((d) => new Date(d.timestamp.toString()))
      .reverse();
    const fundingRates = data.flatMap((d) =>
      exchanges.flatMap((exchange) =>
        assets.map((asset) => {
          const assetData = d[exchange]?.[asset];
          return assetData && assetData.fundingRate
            ? parseFloat(assetData.fundingRate.toString())
            : 0;
        }),
      ),
    );
  ...
  useEffect(() => {
    ...
    g.append("g")
      .call(
        d3
          .axisLeft(yScale)
          .tickFormat((d) =>
            d.replace(/(\w+)-(\w+)-(\w+)_data/i, "$3 $1").toUpperCase(),
          ),
      )
      .selectAll("text")
    ...
    const colorScale = d3
      .scaleSequential(
        d3.interpolateRgbBasis(["red", d3.rgb(213, 100, 247), "blue"]),
      )
      .domain([-colourScalar, colourScalar]);
